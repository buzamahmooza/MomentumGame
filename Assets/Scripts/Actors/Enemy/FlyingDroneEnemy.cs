...
namespace Actors.Enemy
{
    public class FlyingDroneEnemy : ShooterEnemy
    {
        ...
        {
            base.Start();
            InvokeRepeating(nameof(TryToHeal), 0, m_timeBetweenHealing);
        ...
        public override void Attack()
        {
            ...
            base.Attack();
            Invoke(nameof(FireMissile), m_timeBetweenMissiles);
            if (!CanAttack)
                return;
        }
        ...
        private void FireMissile()
        {
            if ((Time.time - m_lastMissileTime) < m_timeBetweenMissiles || !CanAttack)
            {
                ...
                Invoke(nameof(FireMissile), 1); // try again after 1s 
                return;
            }
